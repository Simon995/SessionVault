...
- **[TumeFlow](https://github.com/Simon995/TumeFlow)**（记忆流，Memory Flow）—— 本地优先的 Agent 记忆系统：当前认知持续向前演化，历史变化始终可追溯（`supersede ≠ delete`）。从总库物化自己的固化分库。
...
待办：at-rest 加密 / erase（P3-③ 下游物化 RawEvent→Episode 已在 TumeFlow 落地）。
