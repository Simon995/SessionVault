...
Cursor {
  ...
  # —— provider 专有 ——
  codex_state? {            # 仅 Codex append_log：累计 token 解析所需
    current_model
    current_effort
    current_cwd
    current_session_id
...
4. **截断 / 重写 / 压缩检测**（关键）：文件指纹回退（size 变小、mtime 跳变、或头部哈希变）→ 判定被截断/重写/压缩（如 Codex `history.jsonl` 超 `max_bytes` 丢最旧）→ `safe_offset` 归零、重算尾部窗口，并用 `(occurred_at, content_hash, message_id)` 去重，**不把旧尾当新事件**。
