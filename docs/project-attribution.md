...
其中包含项目搬家**之后**新开的那几条 —— 所以它**不是历史债，是活缺陷**：
每一个在发行版里跑的会话都会中招。

⚠️ 另有 **128 条**是另一回事：它们的 `cwd` 指向注册表建立**之前**就搬走的位置
（`/mnt/<drive>/…`）。库从没见过那些路径，**任何代码都救不回**，`unknown` 就是
正确答案。**两类必须分开**：一类是规则缺失（修一处，以后都对），一类是一次性
历史债（为它写常驻推断逻辑，就是要永远养着的死代码）。
