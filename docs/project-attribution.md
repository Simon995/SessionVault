...
> 🔴 **原文写的是「复核 `scripts/probe-currency-changefeed.py` 仍然通过」，而那个脚本全历史从未入库**（2026-09-02 核）。⇒ 这条「必须复核」**执行不了**，而它读起来像一道已有的闸 —— 于是「提版前复核过了」这句话没有任何东西支撑。**要么把脚本补上，要么这条只能算一句提醒。** 现按后者如实标注。
...
### 🔴 决定 8：读侧要有 `attribute` 出口 —— 归一化不能让每个消费方各写一份（2026-09-01）

**症状。** `roots` 给的是注册表里的**原始写法**，而消费方手上的路径来自别处：
会话记的 `cwd`、用户点的目录。两者指同一个目录却对不上，是常态：

```
注册表（从 Windows 侧发现）  wsl:<distro>:/home/<u>/ws/<repo>
会话 cwd（在那个发行版里跑） /home/<u>/ws/<repo>          ← 直接比较：不匹配
```

而它**不报错**，只表现成「这条会话没有项目」。

🔴 **本节初稿的论据是错的，2026-09-01 当天被另一个消费者用同机数据顶回来 ——
留着订正而不是改写，因为错法比结论有用。**

初稿写的是「281 条 codex 会话里 **31 条**栽在这一条 ⇒ 不是历史债，是活缺陷」。
那 31 条是**直接读原始 `.jsonl`** 量出来的，而摄取路径上
[`pathnorm::normalize_cwd`] **在此之前就把裸 Linux 路径展开了**（宿主是 Windows
且拿得到 `default_distro` 时）：

```rust
if host == HostPlatform::Windows && is_bare_linux_path(raw) {
    if let Some(distro) = default_distro { return Some(format!("wsl:{distro}:{raw}")); }
}
```

⇒ 消费者库里存的 `cwd` **全是规范形**，那三条裸路径精确匹配 **0 行**；
消费侧独立实测：对同一批候选，`attribute` 与 `identify_path` **判决一致（27/27）**。
**我量的那一层没有任何生产消费者在读。**

⇒ **本子命令的受众不是会话流**，而是**持有「任意路径」的消费者**：用户点的目录、
配置里的路径、跨工具传来的 cwd、排查时手工查一条。摄取链路已经在上游做了这件事，
**不要**为它改解析器，也**不要**据此回填历史行。

⚠️ 另有一批（消费侧实测 143 行 / 29 个 cwd）的 `cwd` 指向注册表建立**之前**就搬走
的位置（`/mnt/<drive>/…`）。库从没见过那些路径，**任何代码都救不回**，`unknown`
就是正确答案 —— 尤其**不许用叶子名或路径相似度去猜**（实测：`473` / `5670`
这类数字会撞进完全无关的仓库）。

**决定。** 加只读子命令 `attribute`，判定逻辑落在
`attribution::identify_path`（纯函数、有单测），CLI 只做「开库、取根、拼输出」。

- **四态各有名字**：`resolved` / `no_identity`（找到根了，那个仓没有 remote，
  **接受**）/ `ambiguous`（同样具体的两个根身份不同，**不猜**）/ `unknown`
  （没有任何已知根盖住它，附 `tried` 列出试过的写法）。
- **嵌套根取最具体的那个** —— 否则每个子模块都被判成歧义。同一个仓库的多种写法
  **不是**歧义（身份相同就是同一个项目）。
- **不给 `--distro` 就从注册表已有的 `wsl:<distro>:` 根推**，而不是猜一个默认值：
  推出来的是本机实际见过的发行版；硬编码一个（比如 `Ubuntu`）在别人机器上会
  **安静地一条都展不开**，而那时 `unknown` 里全是本可以认出来的。
- ⚠️ **`/mnt/<drive>/…` 不套发行版前缀** —— 它是挂进来的 Windows 盘，由挂载表
  换算；再套前缀会把同一个目录变成两个键。
- ⚠️ **不做相似度猜测。** 叶子名相同不算证据 —— 消费侧实测过：`473` / `5670`
  这类数字会撞进完全无关的仓库。

**为什么开在这里而不是让消费方自己补。** 消费方补得了，代价是同一条规则出现
N 份。本仓已经为「同一条规则两处实现」付过多次代价（消费侧的 `decode_project_dir`
被删就是其中一次）。与 `roots` 是「项目身份的唯一对外出口」同一个模式。

🔴 **`PathIdentity::Resolved.root` 不是对齐依据。** 见决定 9 的证据：同一个目录在
注册表里可以有多行，于是本函数与 [`attribute`] 对同一条 cwd 会返回**不同写法**的根。
**判「是不是同一个项目」一律用 `canonical_id`**，`root` 只用于展示与排查。

### 🔴 决定 9：注册表**写入侧**要规范化 —— 一个目录只留一行（2026-09-01 人拍板）

**证据（同机实测，两个消费者各自独立撞上）**：同一个项目在注册表里有**三行**，
`canonical_id` 完全相同：
...
全库 11 个 Linux 根里：**同时有裸形式与规范形的 2 个**、只有规范形的 9 个、
只有裸形式的 0 个 —— 也就是说**登记成哪种形式取决于「从哪一侧发现」**。

**后果有三层，一层比一层贵：**

1. 同一个目录**占多行**，各自做一次身份探测、各自算一份 `claude_project_dirs`。
2. 别名机制**是工作的**（第 2 行自己收敛了 `C:\` ⇄ `/mnt/c/`），它只是**登记时
   没查有没有等价的既有根** —— 所以缺的不是机制，是写入时的那次查询。
3. 🔴 **两套判据对同一条 cwd 返回不同写法的根**（一个给 `C:\…`，一个给 `/mnt/c/…`）。
   单独看都对（比较键收敛了），但**返回值是注册表存的原始写法**，于是任何拿它去写
   库的代码都在同一张表里造出第四种写法。消费侧为这一族付过三次代价：同一个物理
   目录占两行、数字劈成两半，**而界面上看不出来**。

⇒ **决定**：登记一个根时，先查有没有**指向同一个物理目录**的既有根；有就**并进
它的 `aliases`**，不新起一行。一个**目录**只留一行。

#### 🔴 判据订正（本节初稿写错了两处，而其中一处照着实现会损坏数据）

初稿写的是「按**比较键**查等价既有根」，度量是「按 **`canonical_id`** 分组、
同一身份下有几种写法，改完应降到 1」。**两处都错**，2026-09-01 由消费侧发现、
两边各自独立复算确认：

🔴 **同一个 `canonical_id` 下的多条根有两种成因，处置相反：**

| | 是什么 | 处置 |
| --- | --- | --- |
| **(a)** | 同一个**物理目录**被登记成多种写法 | 合并 —— 决定 9 要消除的就是它 |
| **(b)** | 同一个**项目**的多份 checkout（submodule / 独立克隆 / 另一侧系统各一份） | 🔴 **合法，绝不能合并** —— 它们是不同的真实目录，内容可以不同 |

实测本机：`git:<host>/<org>/<repo>` 那个身份下有 **4 条根，全是 (b)**。
**照初稿那条度量去「降到 1」，就是把四个真实目录抹成一个。**

而初稿的**判据**（比较键）是另一种错 —— 它**不做发行版展开**，于是
`/home/<u>/ws/<repo>` 与 `wsl:<distro>:/home/<u>/ws/<repo>` 算出两个键，
按它统计得 **0 个重复**，而肉眼可见有两对。

⇒ **正确判据与 [`identify_path`] 的匹配逻辑同源**：把每条根的 `root + aliases`
都过 [`path_forms`]，再取比较键集合；**两条根有共同键 ⇒ 同一个目录**。
两边各自实现、独立跑，同得 **2 组真重复**。

⚠️ **多发行版机器上这条判据会产生歧义**：裸 `/home/<u>/proj` 在两个发行版下展开出
两个候选，可能与两条不同的根都相交 —— 那正是 [`PathIdentity::Ambiguous`] 的形状。
**登记时遇到这种不许猜**：在写入侧错误合并比查询侧的错更难回滚（查询错了下次查就
纠正，合并错了要等到有人发现数据少了）。只报，不合。

⚠️ **度量必须报两个数，不能只报一个**：

```text
🔴 比较键重复的目录：N 个        ← 决定 9 落地后应为 0
ℹ️ 同一项目的不同 checkout：M 个  ← 合法，不该被合并
```

只报第一个的话，第二个数一旦被误读成「还没修干净」，下一个人就会去合并它 ——
**而那正是初稿那条度量会导致的事**。

⚠️ **它动到一个已声明的契约**：`roots` 子命令的文档写着「**不做命名空间翻译**，
给出的是注册表里的原始形式」。规范化之后「原始形式」的含义会变（变成**第一次
登记时**的那种写法），两个消费者都要知道。**这一条不是纯内部重构。**

⚠️ **存量行怎么办是单独一格，而它有两半**：

1. **`root` 值的存量** —— 合并三行会改变消费者今天看到的 `root`。在有迁移方案之前，
   新写入先收敛、存量不动，而**「存量不动」必须能被查出来**（否则「已经规范化了」
   会在存量上悄悄不成立）。
2. 🔴 **`identity_verdict` 的存量**（2026-09-01 由消费侧发现，本节初稿漏了这一半）——
   `identity_verdict` 是 `project_identity_probe` 表里的**存储值，不是读时算的**。
   于是 `CheckoutMissing` 那个修复**上线之后、下一次身份扫描之前**，存量行会继续
   说 `no_identity`。

   **实测**：本机两条已搬走的 checkout 在修复上线当天仍报 `no_identity`
   —— 而那正是这次修复要消除的那句错话（那个仓有 remote，只是目录搬走了），
   它还留在存量里。⇒ **修复生效 ≠ 存量数据更新。**
   ...
   **原预言**（下次身份扫描之后核，一正一反）：
   ...
   | 根 | 预期 | 依据 |
   | --- | --- | --- |
   | 本机路径的那条 | 变成 `checkout_missing` | `find_git_root_with` 的起点 probe 分得出 `StartMissing` |
   | 走桥（WSL）的那条 | **仍是** `no_identity` | 桥那侧 `git_config_path` 只有二态，把「起点不在」压进 `Absent` |

   第一条没变 ⇒ **修复没生效**（不是「本机没有这种数据」）；
   第二条变了 ⇒ 桥那侧其实分得出，上面那条缺口注释该删。

   ##### 🔴 谁来验、怎么算数（**这个验证只有一次机会**）

   **谁**：**消费侧（宿主）**，不是引擎侧。理由不是「谁更近」，是**谁不需要为了
   验证而改用户的数据**：

   | | 引擎侧（`svault` 二进制） | 宿主侧 |
   | --- | --- | --- |
   | `sweep_registered_root_identities` 的生产调用点 | **0 处**（它根本不跑身份扫描） | **唯一一处**，在正常路径上 |
   | 要验证就得 | **专门写一段代码去调它** —— 纯为验证而写入 | 正常用一次就到了，**零额外动作** |

   ⚠️ 附带好处：**写这个修复的人不是唯一验它的人**。但那是附带的，
   前两行才是判据。

   **怎么算数**（不写下来的话，下次扫完没人说得清那两行该不该信）：

   1. 🔴 **必须是新进程**。`identity_seen` 是**进程内**的 `HashSet`，同一进程里
      一个根只探一次（`AlreadyProbed`）。⚠️ 但库里已有的判决**不**阻止重探 ——
      挡住的只有那个内存缓存。
   2. 🔴 **先确认那两条根真的被扫到了**，再看 verdict。`sweep_registered_root_identities`
      有预算截断，`IdentitySweep::skipped_out_of_budget` 会说话。
      **没扫到 ≠ 修复没生效** —— 把两者读成一件事，正是本 ADR 通篇在拆的那种压缩。
   3. 结论只在 1、2 都成立时有效。

**可重复量法**：`svault roots --duplicates`（只读）。判据落在
`attribution::same_directory_groups`（纯函数 + 9 条测试），消费侧探针消费它
—— 那 69 行手写分组实现已删，两份都活着时改一份不会有任何东西报错。

#### 🔴 收尾（2026-09-01）：判据已实现、度量已上线、**自动合并暂无安全对象**

**这不是待办，是结论。** 本机实测：

```text
duplicate_directories   2      ← 两组都是「裸 ⇄ 规范形」
mergeable_duplicates    0      ← 🔴 两组都靠 DistroGuess 才相交
distinct_checkouts      9
ambiguous_overlaps      0
```

两个消费者各自独立实现判据、各自跑，**四个数逐字一致**。

⇒ 在只见过一个发行版的注册表上，我们**没有证据**说那条裸路径属于它，
所以「登记时查等价根」**现在没有可安全执行的对象**。

🔴 **因此不实现它**，理由照抄消费侧的原话：

> 写一段当前无对象可作用的写入侧逻辑，等于让它**第一次真正执行是在某台我们没见过
> 的机器上，而那时没人在场**。

⚠️ **写成待办会招来「顺手做掉」，而这件事恰恰不该顺手做。**

**解锁条件（三条，代价不同）**：

| 条件 | 代价 |
| --- | --- |
| 🟢 注册表记下**发现位置**（只服务于消歧的字段） | **不必推翻决定 3** —— 决定 3 的理由「一个路径是不是项目根，与谁在什么位置扫到它无关」对「是不是根」成立，对「这条裸路径属于哪个发行版」**不成立**。两个问题不同。**这是首选方向** |
| 🟡 一台多发行版机器让 `Ambiguous` 被真实数据驱动 | 被动等待，不可控（现已有构造测试兜着） |
| 🟡 只对 `GroupBasis::Observed` 那一类实现 | 本机那一档已被 alias 机制收敛、零对象；但**在别的机器上可能非零** —— 是「本机没对象」，不是「没用」 |
...
> ⚠️ **原文标注的 `examples/attribution_dryrun.rs` 从未入库**（2026-09-02 核）。下面的数字与它抓出的 bug 都能与本仓其它记录对上（如 `106,814` 那条在 `AGENTS.md` 同样出现），⇒ **实测本身是做过的，丢的是产物**。🔴 但后果实在：**下一个人复现不了这次干跑**，只能重写一份。
...
3. 🔴 **`Unattributed` 的数量要报出来** —— 它是「发现覆盖不到哪里」的度量，
   而不是一个该被藏起来的失败；
4. 重投影后会话记忆**没有**被一次性标红。⚠️ 原文点名 `scripts/probe-currency-changefeed.py`，**该脚本从未入库**（2026-09-02 核）—— 这条验收今天没有现成工具，要自己写；
5. 真库副本上试跑并计时（ADR-044 那次实测「10 分钟跑不完」，四层根因）；
6. 前后各跑一次 source 分布比对。⚠️ 原文点名 `examples/project_root_scope.rs`，**该文件从未入库**（2026-09-02 核）—— 这一步今天要自己写工具。
