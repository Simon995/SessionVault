...
**`NoPath`**，**而不是硬塞一个**。与本 ADR 的主线一致：说不出来就说不出来。

> 🔴 **`NoPath` 与 `Unattributed` 不能混着说**（本节原写作后者，评审 [P2] 指出）。
> 差别正是本 ADR 自己立的那条：`Unattributed` **携带那条路径**（有路径，只是没有已知根
> 覆盖它 —— 下游能按它做粗粒度查询），`NoPath` **什么都不携带**（压根没有路径）。
> 把两者写成一个，等于把「问了、答不出」和「无从问起」说成同一件事 ——
> 而这个 ADR 的全部主张就是这两者必须分开。
...
### 🔴 解码方向重建不出 `_`，而且会**答出另一个真实存在的项目**（2026-08-27）

Claude 的编码是**多对一**：`/` `\` `:` **`_`** 都变成 `-`
（`_` 这一条 2026-08-27 才补进 `encode_project_dir` —— 见下一节的判例）。
所以反过来必然有歧义，而 `decode_project_dir` 是逐层探盘、每层只把成分用 `-`
拼回去 ⇒ **真实目录名里带 `_` 的那一层永远探不到**，于是退到更短的切分。

实测（`decode_cannot_rebuild_an_underscore_yet`）：磁盘上有 `work/my_project` 时，
`work-my-project` 解不出它；而若同一台机器上**恰好还有** `work/my/project`，
返回的就是**后者** —— 一个项目的记忆按**另一个项目**的 git 身份归组。
与本文件「有更长的候选没问成就不能要更短的」是同一个后果，只是触发源不同：
那条是**探测失败**，这条是**候选根本没被生成**。

**为什么不在拼字符串那层修**：枚举 `-`/`_` 组合是 `2^(成分数-1)`，而一条七段的
真实路径在 WSL 桥上每次探测都是一次 `wsl.exe`（本文件别处已为「7 个项目问 70 次」
担心过代价）。加个上限就等于「上限之内是对的，之外照旧答错」——
**一个覆盖不全的检查和没有检查长得一样**。

**正解**：每层**列出子目录**、对每个子目录名 `encode_project_dir` 一次去比。
线性、精确、且对将来新增的映射字符自动成立。代价是 `ProbeBackend` 要加一个
「列目录」方法（`LocalBackend` / `WslBackend` / `WslUncBackend` 三个实现 + 测试假件）
—— 与 `read_text` 当初被提上 trait 是同一个理由：**不在 trait 上，调用方就会
「探测用 A、列目录用 B」**。

⚠️ 在此之前，**消费方一律走编码方向**（`claude_project_dirs` + 存在性检查）。
本文件与 `encode_project_dir` 的抬头早就写着这条；上面这个缺陷只影响
还在走解码的那一处（`class_b.rs` 的快照归属）。

### 🔴 `_` 曾经根本没被当成分隔符（2026-08-27 已修）

`encode_project_dir` 原先只换 `/ \ :`。于是路径里带 `_` 的项目算出来的目录名
**根本不存在** ⇒ 它的 Claude 会话一条都归不到它头上，**且不报错**：消费方看到的是
「这个项目没有会话」，与「真的没跑过」长得一模一样。

判据来自**实测的 `(目录名, cwd)` 对**，不是从字符类别推的：一个消费方机器上抓了
8 对，其中 `<root>/third_party/<name>` 那条的目录名是 `…-third-party-…`（连字符），
而会话正文里的 `cwd` 逐字是 `…\third_party\…`（下划线）。补上这条映射后 8/8 全对；
`.` 等其它字符实测**原样保留**（`Ubuntu-22.04` 里的点还在），所以实现里逐字符列出
要换的那几个，而**不是**「非字母数字一律换」——那样会造出另一批不存在的目录名，
症状完全相同。

⚠️ 那台机器上受影响的量：一个 `third_party/<name>` 项目的 **31 个会话**在消费方
的队列里挂着「未认领」。

### 🔴 TumeChat 的工作记不进 TumeChat

实测（本会话 7,509 行）：提到 TumeChat 路径
