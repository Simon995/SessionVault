...
- **流程**：
  - **step 1（本步）✅**：本契约 + 冻结基线 + `parity` diff 工具 + 首次 parity 报告全部落地，
    首测 must-match=0（见 §8）。`parity` 工具单测随 `cargo test --features parity` 跑。
  - **step 4（后续）**：QuotaBar 改用共享扫描器后，影子并跑 → 用同一 diff 工具比对老/新 `cache.db`，
    must-match 全绿 + advisory 复核通过才切（feature flag，留回退）。
