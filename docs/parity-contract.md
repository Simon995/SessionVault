...
- **流程**：
  ...
  - **step 2 🟡（QuotaBar `p2/svault-shadow` 分支，feature `svault_shadow` 默认关）**：SessionVault 经
    ...
    - `svault_bridge::run_shadow_diff(&Db)`：`discover()+scan(full)` → `project_events` → diff `cache.db`
      两表 + 逐源/逐会话 drill-down 日志。**绿灯只看 `usage_facts` 的 `facts_must_mismatch`**（§1：
      `agent_sessions` 仅二次校验；§6：时间戳 advisory）；`session_diff`（会话 started/last 时间戳）和
      facts 实时增长按 advisory 报告,**不翻红灯**。**只读不写**。
    - **debug 触发（一键可跑）**：后端 Tauri 命令 `svault_shadow_diff`（`#[cfg(feature = "svault_shadow")]`）
      ...
      点按钮 → 界面显示 `ShadowReport`，**`facts_must_mismatch=0` 即绿灯**；session/增长走 advisory 行；
  ...
  - **step 3 🟡（QuotaBar `p2/svault-shadow`，feature `svault_index` 默认关）**：真实索引**写库路径**
    ...
    四种 feature 组合（default / shadow / index / both）均编过。**待办**：写库路径的端到端运行验证
    （本机 Tauri 测试 exe 受限，留 CI/手动 GUI）；之后 **step 4** 影子并跑确认 must-match 全绿 +
    advisory 复核 → 翻默认、拆旧路径。
...
不改变这一点」。三者 ⇒ **step 3 增量写库 facts == 原生 facts**。剩下的端到端（开 `svault_index`
重索引、直接 diff 两份 `cache.db`）等 CI 或手动 GUI（`tauri dev --features svault_index`）补齐。
